...
static void sim_marker(uint16_t*f,int w,int h){const int x=202,y=h-32;rect(f,w,h,x-7,y-6,x+80,y+25,RED);text(f,w,h,x,y,"SIM",4,WHITE);}
...
static void test_overlay(uint16_t*f,int w,int h,const instrument_data_t*d){if(!d->test_overlay)return;rect(f,w,h,70,4,410,70,BLACK);box(f,w,h,70,4,410,70,YELLOW);text(f,w,h,96,11,"TEST MODE - ATTITUDE",2,YELLOW);num(f,w,h,112,35,d->test_index,2,WHITE);text(f,w,h,132,35,"/",2,WHITE);num(f,w,h,144,35,d->test_count,2,WHITE);text(f,w,h,172,35,d->test_name?d->test_name:"TEST",2,WHITE);text(f,w,h,110,55,"TIME",1,WHITE);num(f,w,h,130,55,d->test_seconds_left,1,GREEN);text(f,w,h,142,55,"S",1,GREEN);}
static void pfd(uint16_t*f,int w,int h,const instrument_ui_t*u,const instrument_data_t*d){
    /* The previous implementation called horizon_render_static(), so pitch_deg
       and roll_deg never reached the display. Feed both values into the moving
       horizon renderer; the fixed aircraft symbol remains screen-referenced. */
    ...
    if(d->heading_valid){text(f,w,h,185,87,"HDG",2,WHITE);num(f,w,h,242,87,d->heading_deg,2,GREEN);}if(d->altitude_valid){text(f,w,h,379,184,"ALT",2,WHITE);num(f,w,h,380,215,d->altitude_ft,2,GREEN);}if(!d->altitude_valid)invalid_x(f,w,h,370,190,459,290);if(!d->heading_valid)invalid_x(f,w,h,157,82,323,106);if(!d->attitude_valid)invalid(f,w,h);if(u->settings_active)box(f,w,h,155,414,325,458,YELLOW);test_overlay(f,w,h,d);
}
