...
static void pfd(uint16_t*f,int w,int h,const instrument_ui_t*u,const instrument_data_t*d){horizon_render_static(f,w,h);/* upper roll arc and fixed pointer */for(int a=-60;a<=60;a+=10)radial(f,w,h,(float)a,194,(a%30==0)?224:214,WHITE);ln(f,w,h,w/2,15,w/2-11,42,YELLOW);ln(f,w,h,w/2,15,w/2+11,42,YELLOW);/* reserve side data boxes without pretending values exist */box(f,w,h,362,174,467,306,d->altitude_valid?WHITE:GREY);box(f,w,h,146,18,334,64,d->heading_valid?WHITE:GREY);if(!d->attitude_valid)invalid(f,w,h);if(u->settings_active)box(f,w,h,155,414,325,458,YELLOW);}

void instrument_render(uint16_t*f,int w,int h,const instrument_ui_t*u,const instrument_data_t*d){switch(u->panel){case PANEL_HORIZON:pfd(f,w,h,u,d);break;case PANEL_ALTIMETER:altimeter(f,w,h,u,d);break;case PANEL_COMPASS:compass(f,w,h,u,d);break;default:fill(f,w,h,BLACK);}}
