...
static void radial(uint16_t*f,int w,int h,float deg,int r0,int r1,uint16_t c){float a=(deg-90.0f)*PI_F/180.0f;int cx=w/2,cy=h/2;line(f,w,h,cx+(int)(cosf(a)*r0),cy+(int)(sinf(a)*r0),cx+(int)(cosf(a)*r1),cy+(int)(sinf(a)*r1),c);}

static void render_altimeter(uint16_t*f,int w,int h,const instrument_ui_t*ui,const instrument_data_t*d){fill(f,w,h,BLACK);int r=w/2-28;for(int k=0;k<3;k++)circle(f,w,h,w/2,h/2,r-k,WHITE);for(int i=0;i<50;i++)radial(f,w,h,i*7.2f,r-(i%5?10:22),r,WHITE);if(d->altitude_valid){int a=d->altitude_ft<0?0:d->altitude_ft;radial(f,w,h,(a%1000)*0.36f,0,r-35,WHITE);radial(f,w,h,(a%10000)*0.036f,0,r-62,WHITE);}else invalid_cross(f,w,h);for(int x=175;x<305;x++){pixel(f,w,h,x,355,GREY);pixel(f,w,h,x,395,GREY);}if(ui->settings_active){for(int k=0;k<3;k++)circle(f,w,h,w/2,h/2,r-8-k,YELLOW);}}
static void render_compass(uint16_t*f,int w,int h,const instrument_ui_t*ui,const instrument_data_t*d){fill(f,w,h,BLACK);int r=w/2-30;for(int k=0;k<3;k++)circle(f,w,h,w/2,h/2,r-k,WHITE);for(int i=0;i<36;i++)radial(f,w,h,i*10,r-(i%3?10:22),r,WHITE);line(f,w,h,w/2,22,w/2-10,48,YELLOW);line(f,w,h,w/2,22,w/2+10,48,YELLOW);if(d->heading_valid){radial(f,w,h,(float)(360-d->heading_deg),0,r-38,WHITE);}else invalid_cross(f,w,h);radial(f,w,h,(float)ui->heading_bug_deg,r-34,r-12,YELLOW);if(ui->settings_active){for(int k=0;k<3;k++)circle(f,w,h,w/2,h/2,r-8-k,YELLOW);}}
