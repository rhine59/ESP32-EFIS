...
/* Permanent but compact high-contrast SIM flag whenever displayed values are synthetic. */
static void sim_marker(uint16_t*f,int w,int h){
    const int x=202,y=h-32,s=2;
    rect(f,w,h,x-7,y-6,x+80,y+25,RED);
    /* S */
    rect(f,w,h,x,y,x+17,y+3,WHITE);rect(f,w,h,x,y,x+3,y+11,WHITE);rect(f,w,h,x,y+9,x+17,y+12,WHITE);rect(f,w,h,x+14,y+11,x+17,y+20,WHITE);rect(f,w,h,x,y+18,x+17,y+21,WHITE);
    /* I */
    rect(f,w,h,x+27,y,x+44,y+3,WHITE);rect(f,w,h,x+34,y,x+37,y+21,WHITE);rect(f,w,h,x+27,y+18,x+44,y+21,WHITE);
    /* M */
    rect(f,w,h,x+54,y,x+57,y+21,WHITE);rect(f,w,h,x+72,y,x+75,y+21,WHITE);for(int i=0;i<9;i++){rect(f,w,h,x+57+i,y+i,x+59+i,y+i+s,WHITE);rect(f,w,h,x+70-i,y+i,x+72-i,y+i+s,WHITE);}
}
...
/* PFD: each source has its own unmistakable invalidation region. */
static void pfd(uint16_t*f,int w,int h,const instrument_ui_t*u,const instrument_data_t*d){
    ...
    box(f,w,h,362,174,467,306,d->altitude_valid?WHITE:RED);
    box(f,w,h,146,18,334,64,d->heading_valid?WHITE:RED);
    if(!d->altitude_valid) invalid_x(f,w,h,370,190,459,290);
    if(!d->heading_valid) invalid_x(f,w,h,157,25,323,57);
