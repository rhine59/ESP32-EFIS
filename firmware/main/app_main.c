...
#define QEMU_ALTIMETER_TEST_COUNT 6
...
typedef struct {const char*name;float pressure_hpa;int qnh_hpa;bool valid;} qnh_test_t;
static const qnh_test_t qnh_tests[QEMU_ALTIMETER_TEST_COUNT]={
    {"QNH 1013",927.0f,1013,true},
    {"QNH 1003",927.0f,1003,true},
    {"QNH 1023",927.0f,1023,true},
    {"QNH 950",927.0f,950,true},
    {"QNH 1050",927.0f,1050,true},
    {"PRESS FAIL",0.0f,1013,false}
};
static void apply_qnh_test(const qnh_test_t*t,instrument_ui_t*ui,instrument_data_t*d){ui->qnh_hpa=t->qnh_hpa;ui->settings_active=t->valid;d->altitude_valid=t->valid&&baro_altitude_from_pressure(t->pressure_hpa,(float)ui->qnh_hpa,&d->altitude_ft);d->simulated=true;}
#endif
...
#ifdef CONFIG_EFIS_QEMU
ESP_LOGW(TAG,"QEMU MODE: QNH/PRESSURE ALTIMETER - %d tests",QEMU_ALTIMETER_TEST_COUNT);ui.panel=PANEL_ALTIMETER;int test=0;apply_qnh_test(&qnh_tests[test],&ui,&data);data.test_overlay=true;data.test_name=qnh_tests[test].name;data.test_index=1;data.test_count=QEMU_ALTIMETER_TEST_COUNT;data.test_seconds_left=QEMU_SCENARIO_SECONDS;esp_lcd_panel_handle_t panel=NULL;uint16_t*fb=NULL;ESP_ERROR_CHECK(init_qemu_panel(&panel,&fb));unsigned ticks=0,st=0;while(1){if(++ticks>=5){ticks=0;if(++st>=QEMU_SCENARIO_SECONDS*20){st=0;test=(test+1)%QEMU_ALTIMETER_TEST_COUNT;apply_qnh_test(&qnh_tests[test],&ui,&data);ESP_LOGW(TAG,"QEMU QNH TEST %d/%d: %s pressure=%.1f hPa qnh=%d altitude=%d valid=%d",test+1,QEMU_ALTIMETER_TEST_COUNT,qnh_tests[test].name,(double)qnh_tests[test].pressure_hpa,ui.qnh_hpa,data.altitude_ft,data.altitude_valid);}data.test_overlay=true;data.test_name=qnh_tests[test].name;data.test_index=test+1;data.test_count=QEMU_ALTIMETER_TEST_COUNT;data.test_seconds_left=QEMU_SCENARIO_SECONDS-(int)(st/20);instrument_render(fb,AH_DISPLAY_WIDTH,AH_DISPLAY_HEIGHT,&ui,&data);ESP_ERROR_CHECK(esp_lcd_rgb_qemu_refresh(panel));}vTaskDelay(pdMS_TO_TICKS(10));}
#else
