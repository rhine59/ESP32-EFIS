...
#ifdef CONFIG_EFIS_QEMU
static const instrument_sim_scenario_t tests[QEMU_COMPASS_TEST_COUNT]={SIM_SCENARIO_HEADING_NORTH,SIM_SCENARIO_HEADING_NE,SIM_SCENARIO_HEADING_EAST,SIM_SCENARIO_HEADING_SE,SIM_SCENARIO_HEADING_SOUTH,SIM_SCENARIO_HEADING_SW,SIM_SCENARIO_HEADING_WEST,SIM_SCENARIO_HEADING_NW,SIM_SCENARIO_HEADING_WRAP,SIM_SCENARIO_HEADING_ROTATE,SIM_SCENARIO_HEADING_FAIL,SIM_SCENARIO_HEADING_NORTH};ESP_LOGW(TAG,"QEMU MODE: COMPASS GRAPHICS - %d tests",QEMU_COMPASS_TEST_COUNT);ui.panel=PANEL_COMPASS;ui.settings_active=false;int test=0;instrument_sim_set_scenario(&sim,tests[test]);instrument_sim_step(&sim,0,&data);data.test_overlay=true;data.test_name=instrument_sim_scenario_name(sim.scenario);data.test_index=1;data.test_count=QEMU_COMPASS_TEST_COUNT;data.test_seconds_left=QEMU_SCENARIO_SECONDS;esp_lcd_panel_handle_t panel=NULL;uint16_t*fb=NULL;ESP_ERROR_CHECK(init_qemu_panel(&panel,&fb));unsigned ticks=0,st=0;while(1){if(++ticks>=5){ticks=0;instrument_sim_step(&sim,.05f,&data);if(++st>=QEMU_SCENARIO_SECONDS*20){st=0;test=(test+1)%QEMU_COMPASS_TEST_COUNT;instrument_sim_set_scenario(&sim,tests[test]);instrument_sim_step(&sim,0,&data);ESP_LOGW(TAG,"QEMU COMPASS TEST %d/%d: %s heading=%d bug=%d",test+1,QEMU_COMPASS_TEST_COUNT,instrument_sim_scenario_name(sim.scenario),data.heading_deg,ui.heading_bug_deg);}data.test_overlay=true;data.test_name=instrument_sim_scenario_name(sim.scenario);data.test_index=test+1;data.test_count=QEMU_COMPASS_TEST_COUNT;data.test_seconds_left=QEMU_SCENARIO_SECONDS-(int)(st/20);instrument_render(fb,AH_DISPLAY_WIDTH,AH_DISPLAY_HEIGHT,&ui,&data);ESP_ERROR_CHECK(esp_lcd_rgb_qemu_refresh(panel));}vTaskDelay(pdMS_TO_TICKS(10));}
#else
