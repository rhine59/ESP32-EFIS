...
#define QEMU_HORIZON_TEST_COUNT 12
...
#ifdef CONFIG_EFIS_QEMU
static const instrument_sim_scenario_t tests[QEMU_HORIZON_TEST_COUNT]={SIM_SCENARIO_LEVEL,SIM_SCENARIO_PITCH_UP,SIM_SCENARIO_PITCH_DOWN,SIM_SCENARIO_PITCH_UP_20,SIM_SCENARIO_PITCH_DOWN_20,SIM_SCENARIO_BANK_LEFT,SIM_SCENARIO_BANK_RIGHT,SIM_SCENARIO_BANK_LEFT_60,SIM_SCENARIO_BANK_RIGHT_60,SIM_SCENARIO_COMBINED_UP_RIGHT,SIM_SCENARIO_ATTITUDE_FAIL,SIM_SCENARIO_RECOVERY_LEVEL};ESP_LOGW(TAG,"QEMU MODE: HORIZON GRAPHICS - %d tests",QEMU_HORIZON_TEST_COUNT);ui.panel=PANEL_HORIZON;ui.settings_active=false;int test=0;instrument_sim_set_scenario(&sim,tests[test]);instrument_sim_step(&sim,0,&data);data.test_overlay=true;data.test_name=instrument_sim_scenario_name(sim.scenario);data.test_index=1;data.test_count=QEMU_HORIZON_TEST_COUNT;data.test_seconds_left=QEMU_SCENARIO_SECONDS;esp_lcd_panel_handle_t panel=NULL;uint16_t*fb=NULL;ESP_ERROR_CHECK(init_qemu_panel(&panel,&fb));unsigned ticks=0,st=0;while(1){if(++ticks>=5){ticks=0;instrument_sim_step(&sim,.05f,&data);if(++st>=QEMU_SCENARIO_SECONDS*20){st=0;test=(test+1)%QEMU_HORIZON_TEST_COUNT;instrument_sim_set_scenario(&sim,tests[test]);instrument_sim_step(&sim,0,&data);ESP_LOGW(TAG,"QEMU HORIZON TEST %d/%d: %s pitch=%.1f roll=%.1f",test+1,QEMU_HORIZON_TEST_COUNT,instrument_sim_scenario_name(sim.scenario),(double)data.pitch_deg,(double)data.roll_deg);}data.test_overlay=true;data.test_name=instrument_sim_scenario_name(sim.scenario);data.test_index=test+1;data.test_count=QEMU_HORIZON_TEST_COUNT;data.test_seconds_left=QEMU_SCENARIO_SECONDS-(int)(st/20);instrument_render(fb,AH_DISPLAY_WIDTH,AH_DISPLAY_HEIGHT,&ui,&data);ESP_ERROR_CHECK(esp_lcd_rgb_qemu_refresh(panel));}vTaskDelay(pdMS_TO_TICKS(10));}
#else
