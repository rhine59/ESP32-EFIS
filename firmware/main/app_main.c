...
#define QEMU_ALTIMETER_TEST_COUNT 14
...
#ifdef CONFIG_EFIS_QEMU
static const instrument_sim_scenario_t tests[QEMU_ALTIMETER_TEST_COUNT]={SIM_SCENARIO_ALT_0,SIM_SCENARIO_ALT_500,SIM_SCENARIO_ALT_1000,SIM_SCENARIO_ALT_2500,SIM_SCENARIO_ALT_5000,SIM_SCENARIO_ALT_9500,SIM_SCENARIO_ALT_9900,SIM_SCENARIO_ALT_10000,SIM_SCENARIO_ALT_10100,SIM_SCENARIO_ALT_10500,SIM_SCENARIO_ALT_12500,SIM_SCENARIO_ALTITUDE_SWEEP,SIM_SCENARIO_ALTITUDE_FAIL,SIM_SCENARIO_ALTITUDE_RECOVERY};ESP_LOGW(TAG,"QEMU MODE: ALTIMETER ONLY - %d tests",QEMU_ALTIMETER_TEST_COUNT);ui.panel=PANEL_ALTIMETER;int test=0;instrument_sim_set_scenario(&sim,tests[test]);instrument_sim_step(&sim,0,&data);data.test_overlay=true;data.test_name=instrument_sim_scenario_name(sim.scenario);data.test_index=1;data.test_count=QEMU_ALTIMETER_TEST_COUNT;data.test_seconds_left=QEMU_SCENARIO_SECONDS;esp_lcd_panel_handle_t panel=NULL;uint16_t*fb=NULL;ESP_ERROR_CHECK(init_qemu_panel(&panel,&fb));unsigned ticks=0,st=0;while(1){if(++ticks>=5){ticks=0;instrument_sim_step(&sim,.05f,&data);if(++st>=QEMU_SCENARIO_SECONDS*20){st=0;test=(test+1)%QEMU_ALTIMETER_TEST_COUNT;instrument_sim_set_scenario(&sim,tests[test]);instrument_sim_step(&sim,0,&data);ESP_LOGW(TAG,"QEMU ALTIMETER TEST %d/%d: %s",test+1,QEMU_ALTIMETER_TEST_COUNT,instrument_sim_scenario_name(sim.scenario));}data.test_overlay=true;data.test_name=instrument_sim_scenario_name(sim.scenario);data.test_index=test+1;data.test_count=QEMU_ALTIMETER_TEST_COUNT;data.test_seconds_left=QEMU_SCENARIO_SECONDS-(int)(st/20);instrument_render(fb,AH_DISPLAY_WIDTH,AH_DISPLAY_HEIGHT,&ui,&data);ESP_ERROR_CHECK(esp_lcd_rgb_qemu_refresh(panel));}vTaskDelay(pdMS_TO_TICKS(10));}
#else
