...
#define QEMU_ATTITUDE_TEST_COUNT 6
#define QEMU_SCENARIO_SECONDS 8
...
#else
static esp_err_t init_qemu_panel(esp_lcd_panel_handle_t *panel,uint16_t **fb){const esp_lcd_rgb_qemu_config_t config={.width=AH_DISPLAY_WIDTH,.height=AH_DISPLAY_HEIGHT,.bpp=RGB_QEMU_BPP_16};ESP_RETURN_ON_ERROR(esp_lcd_new_rgb_qemu(&config,panel),TAG,"QEMU RGB panel");ESP_RETURN_ON_ERROR(esp_lcd_panel_reset(*panel),TAG,"QEMU reset");ESP_RETURN_ON_ERROR(esp_lcd_panel_init(*panel),TAG,"QEMU init");void*buffer=NULL;ESP_RETURN_ON_ERROR(esp_lcd_rgb_qemu_get_frame_buffer(*panel,&buffer),TAG,"QEMU framebuffer");*fb=buffer;return ESP_OK;}
#endif
...
void app_main(void){
    ...
    instrument_ui_t ui;instrument_ui_init(&ui);instrument_data_t data={0};instrument_sim_t sim;instrument_sim_init(&sim,BENCH_SIMULATION);instrument_sim_step(&sim,0,&data);
#ifdef CONFIG_EFIS_QEMU
    /* QEMU acceptance work is intentionally one instrument at a time. Keep the
       Horizon/PFD fixed until its test suite and presentation are accepted. */
    static const instrument_sim_scenario_t attitude_tests[QEMU_ATTITUDE_TEST_COUNT]={SIM_SCENARIO_LEVEL,SIM_SCENARIO_PITCH_UP,SIM_SCENARIO_PITCH_DOWN,SIM_SCENARIO_BANK_LEFT,SIM_SCENARIO_BANK_RIGHT,SIM_SCENARIO_ATTITUDE_FAIL};
    ESP_LOGW(TAG,"QEMU MODE: ENABLED - NO PHYSICAL SENSOR OR DISPLAY I/O");
    ESP_LOGW(TAG,"QEMU TEST HARNESS: ATTITUDE/PFD ONLY; %d seconds per test",QEMU_SCENARIO_SECONDS);
    ui.panel=PANEL_HORIZON;instrument_sim_set_scenario(&sim,attitude_tests[0]);instrument_sim_step(&sim,0,&data);
    data.test_overlay=true;data.test_name=instrument_sim_scenario_name(sim.scenario);data.test_index=1;data.test_count=QEMU_ATTITUDE_TEST_COUNT;data.test_seconds_left=QEMU_SCENARIO_SECONDS;
    ESP_LOGW(TAG,"QEMU ATTITUDE TEST 1/%d: %s",QEMU_ATTITUDE_TEST_COUNT,data.test_name);
    esp_lcd_panel_handle_t panel=NULL;uint16_t*fb=NULL;ESP_ERROR_CHECK(init_qemu_panel(&panel,&fb));instrument_render(fb,AH_DISPLAY_WIDTH,AH_DISPLAY_HEIGHT,&ui,&data);ESP_ERROR_CHECK(esp_lcd_rgb_qemu_refresh(panel));
    unsigned ticks=0,scenario_ticks=0;int test=0;
    while(1){if(++ticks>=5){ticks=0;instrument_sim_step(&sim,.05f,&data);data.test_overlay=true;data.test_name=instrument_sim_scenario_name(sim.scenario);data.test_index=test+1;data.test_count=QEMU_ATTITUDE_TEST_COUNT;data.test_seconds_left=QEMU_SCENARIO_SECONDS-(int)(scenario_ticks/20);if(++scenario_ticks>=QEMU_SCENARIO_SECONDS*20){scenario_ticks=0;test=(test+1)%QEMU_ATTITUDE_TEST_COUNT;instrument_sim_set_scenario(&sim,attitude_tests[test]);instrument_sim_step(&sim,0,&data);data.test_name=instrument_sim_scenario_name(sim.scenario);data.test_index=test+1;data.test_seconds_left=QEMU_SCENARIO_SECONDS;ESP_LOGW(TAG,"QEMU ATTITUDE TEST %d/%d: %s",test+1,QEMU_ATTITUDE_TEST_COUNT,data.test_name);}instrument_render(fb,AH_DISPLAY_WIDTH,AH_DISPLAY_HEIGHT,&ui,&data);ESP_ERROR_CHECK(esp_lcd_rgb_qemu_refresh(panel));}vTaskDelay(pdMS_TO_TICKS(10));}
#else
