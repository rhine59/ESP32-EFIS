...
struct ContentView: View {
    ...
    private var instrument:some View{ZStack{Circle().fill(.black);Group{if panel==0{HorizonView(pitch:pitch,roll:roll,valid:attitudeValid,altitude:altitudeValid ? altitude:nil,heading:headingValid ? heading:nil,gpsLatitude:gpsLatitude,gpsLongitude:gpsLongitude,gpsAccuracy:gpsAccuracy,gpsValid:gpsValid,gpsStale:gpsStale)}else if panel==1{AltimeterView(altitude:altitude,qnh:qnh,valid:altitudeValid)}else{CompassView(heading:heading,bug:bug,valid:headingValid,gpsLatitude:gpsLatitude,gpsLongitude:gpsLongitude,gpsAccuracy:gpsAccuracy,gpsValid:gpsValid,gpsStale:gpsStale)}}.clipShape(Circle());Circle().stroke(.gray,lineWidth:5);Text("SIM").font(.headline.bold()).foregroundStyle(.white).padding(.horizontal,12).padding(.vertical,5).background(.red).offset(y:150)}.aspectRatio(1,contentMode:.fit).contentShape(Circle()).onTapGesture{if acceptanceSuite==0 && !demoRunning{panel=(panel+1)%3}}.accessibilityLabel("ESP32 EFIS instrument display")}
