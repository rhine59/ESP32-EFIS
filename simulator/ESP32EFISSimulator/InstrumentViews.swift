...
@ViewBuilder private func invalid(_ s:CGFloat,_ text:String)->some View { ZStack { Path{p in p.move(to:CGPoint(x:s * .22,y:s * .22));p.addLine(to:CGPoint(x:s * .78,y:s * .78));p.move(to:CGPoint(x:s * .78,y:s * .22));p.addLine(to:CGPoint(x:s * .22,y:s * .78))}.stroke(.red,lineWidth:10);Text(text).font(.headline.bold()).foregroundStyle(.red).padding(8).background(.black.opacity(0.9)).overlay(Rectangle().stroke(.red,lineWidth:2)) } }
